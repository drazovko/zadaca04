...
if ($brojRedova) {
    ...
    while ($row = $rezultatUpita->fetch_row()) {
        ...
        $sql2 = " SELECT `idzahtijeva` FROM `zahtijev_za_legalizacijom` "
    ...
    //brojač dokumenata po gradjevinaru po županiji        
        ...
        $output .= '<name ime="' . $row[1] . '" prezime="' . $row[2] . '" zahtijevaPoGradjevinaru="' . $brojacZahtijeva . '" slikaPoGradjevinaru="' . $ukupnoSlikaPoGradjevinaru . '" videaPoGradjevinaru="' . $ukupnoVideaPoGradjevinaru . '" dokumenataPoGradjevinaru="' . $ukupnoDokumenataPoGradjevinaru . '">' . $brojacZahtijeva . '</name>' . "\n";
    }
}
$output .= '</zupanije>';
