...
function pdfStatiskikaDnevnika(){
    korisnik = $('#korisnik').val();
    odDatuma = $('#odDatuma').val();
    doDatuma = $('#doDatuma').val();
    test = $( '#tablica1 td:eq(1)').text();
    ...
    console.log(korisnik, odDatuma, doDatuma, prijave);
    console.log("U funkciji sam 2 pdfStatistikaDnevnika!");
 
  $.ajax(
            {
                type: "GET",
                url: "php_xml/pdfStatiskikaDnevnika.php",
                dataType: 'pdf',
                data:{
                    'korisnik': korisnik,
                    'odDatuma': odDatuma,
                ...
                success: function (data) {
                    console.log("U funkciji sam 3 pdfStatistikaDnevnik!");
                    var tbody = $("<tbody id='fbody1'>");
                    $(data).find('name').each(function() {
                    var red = '<tr>';
                    red += '<td class="prvi_stupac">' + $(this).attr('korisnik') + '</td>';
